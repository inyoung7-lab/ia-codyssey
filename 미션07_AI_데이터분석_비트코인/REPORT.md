...
이 프로젝트의 BTC-USD 데이터는 Yahoo Finance에서 `yfinance`를 통해 수집하여 교육 및 데이터 분석 목적으로 사용했다. 데이터에는 서비스 약관과 재배포 조건이 적용될 수 있으므로 데이터나 결과를 제출·배포할 때는 Yahoo Finance와 원 제공자의 최신 이용 조건을 별도로 확인해야 한다. 확인되지 않은 특정 라이선스는 추정하여 표시하지 않았다.
...
Bitcoin은 24시간 거래되며 단기간에도 큰 가격 변화가 나타날 수 있다. 급등·급락과 변동성의 변화를 충분히 관찰하기 위해 일별 데이터를 분석 단위로 사용했다. 주별 또는 월별로 집계하면 단기 변동이 완화될 수 있으므로 이번 분석 목적에는 일별 단위가 적절하다고 판단했다. 주별·월별 분석을 별도로 수행한 것은 아니다.

### 이상치 판단 및 처리

일별 수익률의 최대·최소값을 포함한 극단적인 가격 변동을 확인했다. 해당 날짜의 날짜 형식, 결측치, 중복 여부와 Open·High·Low·Close 가격이 양수인지, 가격 범위가 비정상적으로 입력됐는지를 점검했다. 확인 결과 분석에 사용한 극단값을 데이터 오류로 판단할 근거가 없었으며 실제 시장에서 발생한 가격 움직임으로 판단했다.

금융 시계열에서는 큰 가격 변동 자체가 중요한 분석 대상이 될 수 있으므로 단순히 값이 크거나 작다는 이유로 제거하지 않았다. 다만 IQR이나 z-score 같은 통계적 이상치 탐지는 수행하지 않았으므로 이상치가 전혀 없다고 단정하지 않는다.

### 트렌드(Trend)

트렌드는 장기간에 걸쳐 나타나는 전체적인 상승 또는 하락 흐름이다. 이번 분석에서는 전체 Bitcoin Close 가격 추세와 MA20, MA60을 이용해 장기적인 가격 흐름과 단기·중기 추세의 변화를 확인했다.

### 계절성(Seasonality)

계절성은 특정 월, 요일, 계절처럼 일정한 시간 간격으로 반복해서 나타나는 패턴이다. 이번 분석은 전체 가격 추세, 수익률, 변동성에 초점을 맞췄기 때문에 월별 또는 요일별 계절성은 별도로 분석하지 않았으며, 계절성이 존재한다고 주장하지 않는다.

### 노이즈(Noise)

노이즈는 장기적인 추세나 반복 패턴으로 설명하기 어려운 단기적이고 불규칙한 변동을 의미한다. Bitcoin의 일별 수익률에 나타나는 급격한 단기 변동에는 이러한 불규칙한 움직임이 포함될 수 있다. 다만 특정 급등·급락은 시장 정보나 외부 요인과 함께 나타날 수도 있으므로 모든 단기 변동을 무조건 노이즈로 판단하지 않는다.
...
가격의 실제 달러 수준과 절대 변화를 직관적으로 비교하기 위해 선형 y축을 사용했다. 다만 기간 중 최저·최고 Close의 차이가 크기 때문에 초기 구간의 상대적 변동이 후기의 큰 가격 수준에 비해 작게 보일 수 있다. 로그 축은 상대 변화 비교에 도움이 될 수 있지만 기존 그래프에는 적용하지 않았으며, 상대 변화는 일별 수익률 그래프와 함께 확인해야 한다.
...
### 5.6 보너스 분석 - 탐색적 시계열 분해

![Bitcoin Close 시계열 분해](images/05_decomposition.png)

`statsmodels.seasonal_decompose`의 가법 모형으로 Close를 관측값, 추세, 계절, 잔차 성분으로 분해했다. Bitcoin이 매일 거래된다는 점을 바탕으로 `period=7`을 주간 반복 가능성을 탐색하기 위한 가정으로 사용했다.

- 사용 관측치: 1,096개
- Trend 유효 관측치: 1,090개(중심 이동평균 특성으로 양 끝 각 3개는 NaN)
- 7일 Seasonal 성분 범위: 371.260735달러
- 전체 Close 범위 대비 Seasonal 성분 범위: 0.343355%
- Residual 표준편차: 1,253.300010달러
- 성분 재결합 최대 오차: 약 0.000000000007달러

Trend는 완만한 장기 흐름, Seasonal은 가정한 7일 주기에 따라 반복되는 성분, Residual은 두 성분으로 설명되지 않은 나머지 변동을 의미한다. 분해된 Trend는 원자료의 장기적인 상승과 기간 중 굴곡을 완만하게 보여준다. 이 설정에서 추정된 7일 Seasonal 성분의 범위는 전체 Close 가격 범위에 비해 작았고, 잔차에는 단기 가격 변동이 남았다. 그러나 이 결과만으로 주간 계절성이 존재하거나 유의하다고 결론 내릴 수는 없다. 가법 분해는 계절 변동 폭이 가격 수준과 무관하게 일정하다고 가정하고, `period=7`도 탐색 목적으로 고정했으며, 통계적 유의성 검정이나 다른 주기와의 비교는 수행하지 않았다.

### 5.7 보너스 분석 - Naive Forecast 기준선

![Bitcoin Close Naive Forecast](images/06_naive_forecast.png)

`Predicted_Close(t) = Actual_Close(t-1)`로 정의한 1단계 Naive Forecast를 기준선으로 사용했다. 최근 구간에서 기준선 오차를 간단히 확인하고 그래프로 비교할 수 있는 고정 길이로 마지막 30개 관측치인 2025-12-02~2025-12-31을 테스트 구간으로 선택했다. 시계열의 선후 관계를 보존하고 미래 관측치가 과거 시점의 예측에 포함되는 것을 막기 위해 데이터를 섞지 않고 시간 순서를 유지했다.

- 테스트 데이터 수: 30개
- MAE: 1,148.341406달러
- RMSE: 1,586.502457달러
- 데이터 누수 점검: 각 예측에는 해당 시점보다 앞선 하루의 실제 Close만 사용

RMSE가 MAE보다 높아 테스트 구간에 평균적인 오차보다 큰 일부 오차가 포함됐음을 확인할 수 있다. 이 방식은 새 모형이 단순히 전일 가격을 사용하는 것보다 나은지 비교하기 위한 최소 기준선이다. 테스트 구간 안에서도 직전 날의 실제값을 매일 알 수 있다고 가정하는 1단계 순차 예측이며, 30일 전체를 한 시점에서 예측하는 다단계 모형이 아니다. 이 예측은 Bitcoin의 미래 가격을 정확하게 예측하기 위한 투자 모델이 아니라 이전 종가를 그대로 다음 날 예측값으로 사용하는 단순 기준선 모델이다. Bitcoin 가격은 뉴스, 거시경제, 정책, 시장 심리 등 다양한 외부 변수의 영향을 받을 수 있으므로 이 결과만으로 미래 가격을 판단할 수 없다.

### 5.8 보너스 기능 - Streamlit 대시보드

기존 CSV를 읽어 사용자가 시작일과 종료일을 선택하고, 선택 기간의 주요 통계와 가격·이동평균·일별 수익률·30일 변동성 그래프를 탐색할 수 있도록 구현했다. 대시보드에서 시작일과 종료일을 변경하면 선택한 기간에 맞게 주요 통계와 시각화가 갱신된다.
...
1. 전체 기간 탐색
   ...
   - 기간: 2023-01-01 ~ 2025-12-31
   - 장기 가격 추세와 전체 기간 주요 지표 확인

   ![전체 기간 Bitcoin 대시보드](images/07_dashboard_full_period.png)

2. 2024년 탐색

   - 기간: 2024-01-01 ~ 2024-12-31
   - 해당 연도의 가격 흐름, 수익률, 변동성 변화 확인

   ![2024년 Bitcoin 대시보드](images/08_dashboard_2024.png)

3. 단기 기간 탐색

   - 기간: 2025-12-20 ~ 2025-12-31
   - 짧은 기간에서도 선택 기간에 따라 지표와 그래프가 갱신되는지 확인

   ![단기 기간 Bitcoin 대시보드](images/09_dashboard_short_period.png)
...
- 추가 분석(Action): 향후 현물 Bitcoin ETF의 일별 순유입·순유출 데이터와 Bitcoin 변동성을 함께 비교하면 ETF 자금 흐름과 변동성 사이의 관계를 추가로 검증할 수 있다.
...
- 추가 분석(Action): S&P 500 또는 Nasdaq과 같은 위험자산의 일별 수익률과 Bitcoin 일별 수익률을 함께 비교하면 글로벌 금융시장 움직임과 Bitcoin의 관계를 추가로 분석할 수 있다.
...
- 추가 분석(Action): 정책 발표 전후의 시간 단위 Bitcoin 가격 데이터를 추가로 분석하면 뉴스 발표 직후 가격 반응을 일별 데이터보다 더 세밀하게 확인할 수 있다.
...
- Streamlit 대시보드 구조와 예외 처리 코드 작성 보조
- 7일 주기 시계열 분해 코드와 검증 항목 작성 보조
- Naive Forecast 기준선, MAE·RMSE 계산 및 데이터 누수 점검 코드 작성 보조
...
- 분석 방법의 대안 탐색
- 오류 가능성 감소
- 보너스 분석의 가정과 한계를 명시하고 재현 가능한 검증 절차를 구성하기 위해 사용
...
- 대시보드의 전체·연간·단기 기간 선택 시나리오와 예외 상황을 함수 단위로 확인
- 시계열 분해 성분의 재결합 오차와 결과 이미지 생성을 확인
- Naive Forecast의 시간 순서, 직전 실제값 사용, 테스트 30개와 MAE·RMSE를 직접 확인
- 기존 CSV의 실행 전·후 SHA-256을 비교해 원본이 변경되지 않았음을 확인

### 활용 원칙

AI가 제시한 해석을 그대로 사실로 사용하지 않았으며, 데이터에서 확인한 관찰(Fact)과 추가 검토가 필요한 해석(Hypothesis)을 구분했다. 보너스 결과의 수치도 AI가 임의 생성한 값이 아니라 Python 실행 결과와 검증 출력에 근거해 기록했다.
